...
def test_boto_backend_removes_managed_alias_and_qualifier_configuration():
    environment, clients = boto_environment()
    clients["lambda"].list_tags.return_value = {
        "Tags": {"taskflows:managed-alias": "live", "taskflows:managed-tag-keys": "[]"}
    }
    ...
    assert result.success
    clients["lambda"].delete_alias.assert_called_once_with(FunctionName="demo", Name="live")
    clients["lambda"].delete_function_event_invoke_config.assert_any_call(
        FunctionName="demo", Qualifier="live"
    )
    clients["lambda"].delete_provisioned_concurrency_config.assert_any_call(
        FunctionName="demo", Qualifier="live"
    )


def test_boto_backend_reconciles_removed_user_tags():
    environment, clients = boto_environment()
    clients["lambda"].list_tags.return_value = {
        "Tags": {
            "Old": "value",
            "taskflows:managed-tag-keys": '["Old"]',
            "taskflows:managed-alias": "",
        }
    }

    result = environment.deploy_function(
        packaged_function,
        CloudFunctionConfig(function_name="demo", auto_create_role=False, tags={"New": "value"}),
    )

    assert result.success
    clients["lambda"].untag_resource.assert_called_once_with(Resource=ANY, TagKeys=["Old"])
    tags = clients["lambda"].tag_resource.call_args.kwargs["Tags"]
...
@pytest.mark.parametrize(
    "target_arn",
    [
        "arn:aws:sqs:us-east-1:123:dead-letter.fifo",
        "arn:aws:sns:us-east-1:123:dead-letter.fifo",
    ],
)
def test_lambda_rejects_fifo_dead_letter_targets(target_arn):
    with pytest.raises(ValueError, match="standard"):
        config = CloudFunctionConfig(
            function_name="demo",
            dead_letter_config=DeadLetterConfig(target_arn=target_arn),
        )
        validate_aws_lambda_config(config)


def test_boto_backend_does_not_modify_an_unowned_execution_role():
    environment, clients = boto_environment()
    environment.config.execution_role_arn = None
    clients["iam"].get_role.return_value = {"Role": {"Arn": "arn:aws:iam::123:role/existing"}}
    clients["iam"].list_role_tags.return_value = {"Tags": []}
    ...
    assert "not managed by Taskflows" in (result.error or "")
    clients["iam"].attach_role_policy.assert_not_called()
