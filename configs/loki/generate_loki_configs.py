...
def generate_config(
    ...
    if not validate_config(profile, profile_name) and not expand_vars:
        print(
            "\nNote: Validation failed. This might be expected if environment variables are not expanded.",
            file=sys.stderr,
        )
        print(
            "      Use --expand-env to expand environment variables before validation.",
            file=sys.stderr,
        )
...
Examples:
  ...
  %(prog)s --profile s3-standard --no-expand-env
    ...
    parser.add_argument("--profile", "-p", help="Profile name to generate configuration for")
    parser.add_argument("--output", "-o", help="Output file path (default: stdout)")
    parser.add_argument(
        "--list-profiles", "-l", action="store_true", help="List all available profiles"
    )
    parser.add_argument(
        "--all", "-a", action="store_true", help="Generate configurations for all profiles"
    )
    parser.add_argument(
        "--output-dir",
        "-d",
    ...
    parser.add_argument(
        "--template",
        "-t",
        default="loki-config.j2",
        help="Jinja2 template file (default: loki-config.j2)",
    )
    parser.add_argument(
        "--profiles-file",
        "-f",
        default="loki-profiles.yaml",
    ...
    if args.list_profiles:
        ...
        for profile_name in profiles:
            ...
            if generate_config(
                profile_name, profiles, str(template_file), str(output_file), not args.no_expand_env
            ):
                success_count += 1
        ...
        success = generate_config(
            args.profile, profiles, str(template_file), args.output, not args.no_expand_env
        )
