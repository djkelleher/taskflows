...
async def send_discord_message(
    ...
    retries: int = 3,
    attachment_files: Sequence[str | Path | AttachmentFile] | None = None,
    **kwargs,
) -> bool:
    ...
    processed_attachments = [zip_attachment] if zip_attachment else individual_attachments

    # Discord has a character limit for messages
    # If we have attachments or message is not too long, send single message
    if processed_attachments or len(message_text) <= discord_settings.message_max_length:
        ...
        try:
            async with aiohttp.ClientSession(timeout=timeout) as session:
                if attachments:
                    ...
                    for i, attachment in enumerate(attachments[: discord_settings.max_attachments]):
            ...
            logger.error(f"Discord webhook request failed on attempt {attempt + 1}: {e}")

        if attempt < retries:
            await asyncio.sleep(discord_settings.retry_base_delay**attempt)  # Exponential backoff

    logger.error(f"Failed to send Discord message after {retries + 1} attempts")
    return False
