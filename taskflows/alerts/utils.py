...
def use_inline_tables(tables: Sequence["Table"], inline_tables_max_rows: int) -> bool:
    ...
    return sum(len(t.rows) if t.rows is not None else 0 for t in tables) < inline_tables_max_rows
...
def attach_tables(tables: Sequence["Table"], attachments_max_size_mb: int) -> bool:
    ...
    if tables:
        tables_size_mb = (
            sum(sys.getsizeof(t.rows) if t.rows is not None else 0 for t in tables) / 10**6
        )
...
def prepare_attachments(
    attachment_files: Sequence[str | Path | AttachmentFile] | None,
    force_zip: bool = False,
    size_threshold_mb: float = 20.0,
    zip_filename: str | None = None,
) -> tuple[list[AttachmentFile], AttachmentFile | None]:
