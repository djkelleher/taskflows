...
error_text = Text(
    value="Database Connection Failed", level=ContentType.ERROR, font_size=FontSize.MEDIUM
)
```
...
data_map = Map(data={"CPU Usage": "85%", "Memory": "12.5 GB", "Disk Space": "45% free"})

# Inline key-value display
inline_map = Map(data={"Status": "Online", "Version": "2.1.0", "Uptime": "15 days"}, inline=True)
```
...
# Unordered list
todo_list = List(items=["Review pull requests", "Update documentation", "Deploy to staging"])

# Ordered list
steps_list = List(
    items=["Install dependencies", "Run tests", "Build application", "Deploy to production"],
    ordered=True,
)
```
...
# Button-style link
button_link = Link(url="https://dashboard.example.com", text="Open Dashboard", button_style=True)
```
...
chart_image = Image(
    src="https://example.com/analytics.png", alt="Analytics Dashboard", width=800, height=600
)
```
...
timeline = Timeline(
    events=[
        ...
        {"time": "10:30 AM", "title": "Database Backup", "description": "Daily backup completed"},
        {"time": "12:00 PM", "title": "User Activity Peak", "description": "High traffic detected"},
    ]
)
```
...
custom_progress = ProgressBar(
    value=60, max_value=80, color="green", show_percentage=False, width=300
)
```
...
error_alert = Alert(
    message="Database connection failed", alert_type="error", title="Connection Error"
)
...
full_quote = Quote(
    text="Stay hungry, stay foolish.", author="Steve Jobs", source="Stanford Commencement Speech"
)
```
...
breadcrumb = Breadcrumb(items=["Home", "Dashboard", "Analytics", "Reports"])
```
...
avatar_with_image = Avatar(
    name="John Doe", image_url="https://example.com/avatar.jpg", size="large"
)
...
discord_logger = await AlertLogger.create(
    channel="https://discord.com/api/webhooks/your-webhook-url", msg_max_freq=2
)
