...
MsgDst = EmailAddrs | SlackChannel | DiscordChannel


@dataclass
class PeriodicMsgs:
    send_to: MsgDst | Sequence[MsgDst]
    msg_buffer: list[str | Component] = field(default_factory=list)
    on_pub_func: Callable | None = None
    header: str | None = None

    def add_message(self, message: str | Component):
...
class AlertLogger:
    ...
    def __init__(self):
        self.channel: SlackChannel | DiscordChannel | None = None
        self.disable_messages: bool = False
        self.msg_max_freq: int = 2
        self._log: Callable | None = None
        self._periodic_sender: PeriodicMsgSender | None = None
        self._periodic_msgs: PeriodicMsgs | None = None
    ...
    async def add_periodic_pub_group_member(
        self, config: PeriodicMsgs, pub_freq: int | None = None
    ):
    ...
    **kwargs,
    ...
    for component in content:
        if isinstance(component, Text):
            ...
            if attachment_info:
                ...
                attachment_files.append(AttachmentFile(filename=filename, content=file_obj))

    # Add attachment files to kwargs if any exist
    if attachment_files:
        kwargs["attachment_files"] = kwargs.get("attachment_files", []) + attachment_files
    ...
    for i, result in enumerate(sent_ok):
        if isinstance(result, Exception):
            ...
            logger.warning(f"Unexpected return type from send alert: {type(result)} = {result!r}")
