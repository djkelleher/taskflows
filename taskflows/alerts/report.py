...
class Report:
    ...
    def __init__(self, *components: Component | str | Sequence[Component | str]):
        """
        Args:
    ...
    def add(self, *components: Component | str | Sequence[Component | str]) -> "Report":
    ...
    async def image_async(
        self, theme: str = "dark", executor: ThreadPoolExecutor | None = None
    ) -> BytesIO:
        ...
        Returns:
            BytesIO: The image data as bytes.
        """
        return await render_components_image_async(self.components, theme=theme, executor=executor)
...
def render_components_image(components: Sequence[Component], theme: str = "dark") -> BytesIO:
...
async def render_components_image_async(
    ...
    if executor is None:
        ...
        return await loop.run_in_executor(executor, render_components_image, components, theme)
...
def render_components_md(
    ...
    return "\n\n".join(
        [c.md(slack_format=slack_format, discord_format=discord_format) for c in components]
    ).strip()


def _components_list(components: Sequence[Component]) -> list[Component]:
