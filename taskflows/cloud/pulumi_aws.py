...
"""
    ...
    def _create_lambda_infrastructure(
        ...
        if not vpc_config and config.security_group_ids and config.subnet_ids:
            vpc_config = {
                "security_group_ids": config.security_group_ids,
                "subnet_ids": config.subnet_ids,
            }
        if vpc_config:
            # Pulumi uses snake_case while the direct AWS backend accepts the
            # botocore shape.
            lambda_args["vpc_config"] = {
                "security_group_ids": vpc_config.get(
                    "SecurityGroupIds", vpc_config.get("security_group_ids", [])
                ),
                "subnet_ids": vpc_config.get("SubnetIds", vpc_config.get("subnet_ids", [])),
            }
        ...
        if config.log_retention_days:
            log_group_args["retention_in_days"] = config.log_retention_days
    ...
    def _runtime_backend(self):
        """Return the boto3 facade used for operations Pulumi does not model."""
        from .aws_lambda import AWSLambdaConfig, AWSLambdaEnvironment

        backend = getattr(self, "_runtime_backend_instance", None)
        if backend is None:
            backend = AWSLambdaEnvironment(AWSLambdaConfig(region=self.region))
            self._runtime_backend_instance = backend
        return backend
    ...
    def rollback_function(
        ...
        """Move an AWS Lambda alias; Pulumi has no separate alias rollback API."""
        return self._runtime_backend().rollback_function(function_name, version, rollback_id)
    ...
    def get_function_metrics(
        ...
        return self._runtime_backend().get_function_metrics(function_name, start_time, end_time)
    ...
    def create_layer(
        ...
        return self._runtime_backend().create_layer(layer_config, requirements_file)

    def list_versions(self, function_name: str) -> list[dict[str, Any]]:
        return self._runtime_backend().list_versions(function_name)

    def set_function_alias(self, function_name: str, alias_name: str, version: str) -> bool:
        return self._runtime_backend().set_function_alias(function_name, alias_name, version)
